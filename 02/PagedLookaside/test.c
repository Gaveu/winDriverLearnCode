...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegistryPath)
{
	...
	int i = 0;
	PAGED_LOOKASIDE_LIST pLList;
	ExInitializePagedLookasideList(
		&pLList,		//PPAGED_LOOKASIDE_LIST		Lookaside	待初始化的快查表
	...
	for (i = 0; i < 10; ++i)
	{
		PMYDATA pTmp = (PMYDATA)ExAllocateFromPagedLookasideList(&pLList);
		if (!pTmp)
		{
			KdPrint(("PagedLookasideList Allocate Failed!\n"));
	...
	{	//逐个输出分配的内存块的数据并回收其空间
		...
		ExFreeToPagedLookasideList(
			&pLList,	//PPAGED_LOOKASIDE_LIST	Lookaside	待回收内存块所在的快查表
			pData[i]	//PVOID					Entry		指向待回收内存块首地址的指针
			);
	}

	ExDeletePagedLookasideList(&pLList);
